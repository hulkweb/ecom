...
@section('content')
                ...
                <div class="sumary-product single-layout">
                    <div class="media">
                        <ul class="biolife-carousel slider-for"
                            ...
                            <li><img src="/uploads/products/{{ $product->image }}" alt="" width="500"
                                    height="500"></li>
                            @foreach ($product_images as $item)
                                <li><img src="/uploads/products/{{ $item->image }}" alt="" width="500"
                                        height="500"></li>
                            @endforeach
                        </ul>
                        <ul class="biolife-carousel slider-nav"
                            ...
                            <li><img src="/uploads/products/{{ $product->image }}" alt="" width="88"
                                    height="88"></li>
                            @foreach ($product_images as $item)
                                <li><img src="/uploads/products/{{ $item->image }}" alt="" width="500"
                                        height="500"></li>
                    ...
                    <div class="product-attribute">
                        <h3 class="title text-capitalize">{{ $product->name }}</h3>
                        <div class="rating">
                            <p class="star-rating"><span class="width-80percent"></span></p>
                            <span class="review-count">({{ count($reviews) }} Reviews)</span>
                            <span class="qa-text">Q&A</span>
                            <b class="category">By: {{ $product->user ? $product->user->name : 'Admin' }}</b>
                        </div>
                        <span class="sku">Sku: #76584HH</span>
                        <p class="excerpt">{{ $product->details }}</p>
                        <div class="price">
                            <ins><span class="price-amount"><span
                                        class="currencySymbol">₹</span>{{ $product->price }}.00</span></ins>
                            <del><span class="price-amount"><span
                                        class="currencySymbol">₹</span>{{ $product->price + 100 }}.00</span></del>
                        </div>
                    ...
                    <div class="action-form">
                        <form action="/add_to_cart" method="POST">
                            ...
                            @else
                                <div class="quantity-box">
                                    <span class="title text-danger text-bold">Out of stock</span>
                                </div>
                            @endif
                                ...
                                <div class="row">
                                    <div class="col-lg-5 col-md-5 col-sm-6 col-xs-12">
                                        <div class="rating-info">
                                            ...
                                            <p class="see-all">See all {{ count($reviews) }} reviews</p>
                                            <ul class="options">
                                                <li>
                                    ...
                                    <div class="col-lg-7 col-md-7 col-sm-6 col-xs-12">
                                        <div class="review-form-wrapper">
                                            ...
                                            @if (!auth()->user())
                                                <form action="{{ route('reviews.store') }}" name="frm-review"
                                                    method="post">
                                                    @csrf <div class="comment-form-rating">
                                                    ...
                                                    <p class="form-row wide-half">
                                                        <input type="text" name="title" value=""
                                                            placeholder="Overall title">
                                                    </p>
                                                    <input type="hidden" value="{{ $product->id }}" name="product_id">
                                                    <input type="hidden"
                                                        value="{{ auth()->user() ? auth()->user()->id : 1 }}"
                                                        name="user_id">
                                                    <p class="form-row">
                                                        <textarea name="details" id="txt-comment" cols="30" rows="10" placeholder="Write your review here..."></textarea>
                                                    </p>
                                                    <p class="form-row">
                                            ...
                                            @else
                                                <a href="/login">Please Login to submit your review</a>
                                            @endif
                                                    ...
                                                    <div class="row">
                                                        <div class="comment-content col-lg-8 col-md-9 col-sm-8 col-xs-12">
                                                            <p class="comment-in"><span
                                                                    class="post-name">{{ $item->title }}</span><span
                                                                    class="post-date">{{ date('d/m/y', strtotime($item->created_at)) }}</span>
                                                            </p>
                                                            <div class="rating">
                                                                <p class="star-rating"><span
                                                                        class="width-40percent"></span>
                                                                </p>
                                                            </div>
                                                            <p class="author">by: <b>{{ $item->user->name }}</b></p>
                                                            <p class="comment-text">{{ $item->details }}</p>
                                                        </div>
                                                        <div
                                                            ...
                                                            <ul class="actions">
                                                                <li><a href="/reviews/helpfull/{{ $item->id }}"
                                                                        class="btn-act like" data-type="like"><i
                                                                            class="fa fa-thumbs-up"
                                                                            aria-hidden="true"></i>Yes
                                                                        ({{ $item->helpful }})
                                                                    </a></li>
                                                                <li><a href="/reviews/unhelpfull/{{ $item->id }}"
                                                                        class="btn-act hate" data-type="dislike"><i
                                                                            class="fa fa-thumbs-down"
                                                                            aria-hidden="true"></i>No
                                                                        ({{ $item->no_helpful }})</a></li>
    ...
    <script>
        $(".btn-rating").click(function() {
            $("input[name=rating]").val($(this).attr("data-value").split("-")[1])
        })
    </script>
@endsection
